    ...
    def replace_tag(self, media_type: Literal['image', 'video', 'audio'], index: int,
        ...
        if media_type == 'image':
            ...
            try:
                ...
                video_path = video if isinstance(video, str) else f'video_index_{index}'
                logger.warning(f'[VideoReadError] 视频读取失败，将使用fallback视频代替: {video_path}, 错误: {e}')
                # 将坏视频路径写入日志文件，方便后续过滤
                failed_log = os.environ.get('FAILED_VIDEO_LOG', os.path.join(os.getcwd(), 'failed_videos.log'))
                try:
                    with open(failed_log, 'a') as f:
                        f.write(f'{video_path}\n')
                except Exception:
                    pass
                # 直接使用环境变量FALLBACK_VIDEO_PATH指定的正常视频
                fallback_path = os.environ.get('FALLBACK_VIDEO_PATH', '')
                fallback_inputs = {'video': fallback_path, **inputs.chat_template_kwargs}
                video, video_kwargs = fetch_video(fallback_inputs, return_video_sample_fps=True, **kwargs)
                logger.warning(f'[VideoReadError] 已使用fallback视频: {fallback_path}')
