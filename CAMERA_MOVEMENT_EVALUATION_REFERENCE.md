...
**`segment_matching` vs `matched_basic_movement_with_direction` (commonly confused)**

The `segment_level` result dict contains two conceptually different metric groups. They measure orthogonal things and must be read together:

| Metric key | Measures | Basis (denominator) | Question it answers |
|---|---|---|---|
| `segment_matching` | **Temporal localization only** — content-agnostic | all predicted / all GT *segments* | "Did I cut the timeline into the right intervals?" |
| `matched_basic_movement_with_direction` | **Movement-label accuracy** (type + direction) | *labels* inside the already-matched pairs | "Given a correctly localized segment, did I name the right movement + direction?" |

**1. `segment_matching`** — temporal segmentation quality
- Ignores *what* movement is inside a segment; only checks whether a predicted time interval overlaps a GT interval with `IoU >= threshold` (default 0.5), via the greedy matching above.
- `precision = total_matched / total_pred_segments`, `recall = total_matched / total_gt_segments`, `f1 = 2PR/(P+R)`.
- A spurious extra segment lowers precision; a missed GT segment lowers recall. **The movement label is irrelevant here** — even a totally wrong label still "matches" as long as the time spans overlap enough.

**2. `matched_basic_movement_with_direction`** — labeling accuracy, conditioned on matching
- Computed **only over the (GT, Pred) pairs that already passed IoU matching**. Missed / spurious segments are *not* counted here (they are already penalized by `segment_matching`).
- For each matched pair, both sides are turned into a **set of compound labels** (via `get_basic_movement_labels`):
  - directional types (Pan/Tilt/Truck/Crane/Arc/Roll) → `"type_direction"`, e.g. `"Pan_left"`, `"Crane_up"`
  - non-directional types (Static / Dolly In / Zoom In …) → just the `type`
- Multi-label **set-based** P/R/F1, accumulated across all matched pairs: `TP = |gt ∩ pred|`, `FP = |pred − gt|`, `FN = |gt − pred|`. So getting the type right but the direction wrong (`Pan_left` vs `Pan_right`) scores as **1 FP + 1 FN**, not a partial match.

**Why both exist / how to interpret**
- `segment_matching` isolates **where** (temporal boundaries); `matched_basic_movement_with_direction` isolates **what** (movement semantics), *assuming* localization was already correct.
- Because the label metric is conditioned on matched pairs, a model can show **high** `matched_basic_movement_with_direction` yet **low** `segment_matching` — it labels well the few segments it does localize, but localizes poorly overall. Neither number alone tells the full story.
- Contrast with the frame-level `basic_movement_with_direction`, which *entangles* localization and labeling by scoring every 0.1 s frame. The segment-level `matched_*` metrics deliberately **decouple** the two.

**Sibling `matched_*` metrics** (same "matched pairs only" basis):
- `matched_basic_movement_type_only` — same as above but **ignoring direction** (type only); compare against `matched_basic_movement_with_direction` to isolate direction errors.
- `matched_special_movement` — special-movement multi-label P/R/F1.
- `matched_speed` — speed accuracy over matched pairs.
